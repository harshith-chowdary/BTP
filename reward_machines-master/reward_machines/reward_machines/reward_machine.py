    ...
    def save(self, save_path):
        ...
        with open(save_path, 'wb') as f:
            pickle.dump({
                ...
                'known_transitions': self.known_transitions,
                'gamma': self.gamma,
                'potentials': self.potentials
            }, f)
        print(f"RewardMachine saved to {save_path!r}")

    @classmethod
    def load(cls, save_path):
        with open(save_path, 'rb') as f:
            data = pickle.load(f)
        print(f"RewardMachine loaded from {save_path!r}")
        rm = cls.__new__(cls)
        rm.U = data['U']
        rm.u0 = data['u0']
        rm.delta_u = data['delta_u']
        rm.delta_r = data['delta_r']
        rm.terminal_u = data['terminal_u']
