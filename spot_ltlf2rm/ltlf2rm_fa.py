...
if len(sys.argv) < 2:
    print("Usage: python ltlf2rm_fa.py <automaton type>")
    print("Default: Non - Deterministic Bucchi Automaton")
    det = 0
else:    
    det = sys.argv[1]

def ltlf_to_fa(ltlf_formula):
...
# ltl_list = [
#     'F (a & F (b & F c))',  # Reachability
#     'F (f & g)',            # Conjunction of Events
#     'G (! x)',               # Safety Condition
#     'G (m -> F n)',          # Sequential Events
#     '(! k) U j',             # Avoidance and Reachability
#     'G (! (p1 & p2))',       # Mutual Exclusion
#     'G (a -> G b)',          # Eventual Safety
#     'GF q -> GF r',          # Strong Response
#     'G (t -> X u)',          # Next State Condition
#     'G (s -> F v)'           # Repeated Eventuality
# ]

# reward_list = [1, 2, 3, 2, 4, 1, 3, 5, 2, 4]

# STEP 2: Transform LTLf formulas into fa
automata = [ltlf_to_fa(ltlf_formula) for ltlf_formula in ltl_list]

# STEP 3: Do the cross product automaton
product_fa = terf_product(automata, reward_list)
# rm_hoa = product_fa.to_str('hoa', 't')
rm_hoa = product_fa.to_str('hoa') # HOA to string
